...
const SUPABASE_KEY = "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6Ind6ZHRiZm1oc2psY2hsbWtsdGNoIiwicm9sZSI6InNlcnZpY2Vfcm9sZSIsImlhdCI6MTc2NTcyNDM5NywiZXhwIjoyMDgxMzAwMzk3fQ.JSOpMQ0hwllMw5UXc6hedt4odsZgynFV1_4j6ZTY-W8"; // server key for uploads
...
const QUERIES = [
  "art", "design", "background", "wallpaper", "abstract",
  "pattern", "texture", "nature", "landscape", "animals",
  "people", "portrait", "technology", "object", "flowers",
  "scenery", "fantasy", "digital", "surreal", "illustration",
  "colorful", "environment", "minimalist", "creative",
  "city", "building", "sky", "shape"
];
// ================================================
...
function detectCategories(tagsArray) {
  ...
  if (/nature|forest|tree|mountain|river|landscape/i.test(text)) categories.push("nature");
  if (/person|people|woman|man|face|portrait/i.test(text)) categories.push("people");
  if (/city|building|architecture|urban|street/i.test(text)) categories.push("buildings");
  if (/food|fruit|vegetable|meal|dish/i.test(text)) categories.push("food");
  if (/computer|tech|technology|device/i.test(text)) categories.push("technology");
